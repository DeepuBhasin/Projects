...
import { getSingleAccountAction, updateAccountAction } from "./../../redux/accountSlice"

const EditAccount = () => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const { account, loading, error, isUpdated } = useSelector((state) => state?.accountData);
  ...
  return (
    <React.Fragment>
      {loading ? <h2 className="text-center text-green-600 mt-5 text-lg">Loading...</h2> : error ? <h2 className="text-center text-red-600 mt-5 text-lg">{error}</h2> : <section className="py-16 xl:pb-56 bg-white overflow-hidden">
